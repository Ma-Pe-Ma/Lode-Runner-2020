...
uint32_t JNIHelper::LoadTexture(const char* file_name, int32_t* outWidth,
  ...
  jclass javaCls =
      RetrieveClass(env, "com/mpm/helper/NDKHelper$TextureInformation");
...
AAssetManager* JNIHelper::GetAssetManager() {
  return activity_->assetManager;
}

void JNIHelper::setLastLevel(int lastLevel) {
    if (activity_ == NULL) {
        LOGI(
                "JNIHelper has not been initialized. Call init() to initialize the "
                "helper");
    ...
    JNIEnv* env = AttachCurrentThread();
    jmethodID mid = env->GetMethodID(jni_helper_java_class_, "setLastLevel", "(I)V");
    if (mid == NULL) {
        LOGI("method ID %s, '%s' not found", "sendShorts", "([S)V");
        return;
    }

    env->CallVoidMethod(jni_helper_java_ref_, mid, (jint) lastLevel);
...
extern "C" {
JNIEXPORT void Java_com_mpm_helper_NDKHelper_RunOnUiThreadHandler(
...
JNIEXPORT void Java_com_mpm_helper_NDKHelper_MessageFromNDK(JNIEnv* env, jobject thiz) {
